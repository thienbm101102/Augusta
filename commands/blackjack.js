...
// Giả định bạn đã tạo một Schema cho game Xì Zách để lưu vào MongoDB
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    // Kiểm tra xem người dùng đã có game đang diễn ra chưa
    ...
    // Lưu trạng thái game vào MongoDB
  ...
  async handleButton(interaction) {
    ...
    if (interaction.customId === 'xizach_hit') {
      ...
      if (dealerVal > 21 || playerVal > dealerVal) {
        await db.addBalance(userId, game.bet * 2);
        newBalance = await db.getBalance(userId);
        ...
        await db.addBalance(userId, game.bet);
        newBalance = await db.getBalance(userId);
    ...
    if (ended) {
      ...
      // Cập nhật trạng thái game trong MongoDB
      await game.save();
